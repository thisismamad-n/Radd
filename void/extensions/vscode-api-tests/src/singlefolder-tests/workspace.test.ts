...
suite('vscode API - workspace', () => {
	...
	test('openTextDocument, actual casing first', async function () {
		...
		fs.writeFile(uriOne, Uint8Array.from(Buffer.from('one')), { create: true, overwrite: true });
		fs.writeFile(uriTwo, Uint8Array.from(Buffer.from('two')), { create: true, overwrite: true });
	...
	test('Support creating binary files in a WorkspaceEdit', async function (): Promise<any> {
		...
		const data = Uint8Array.from(Buffer.from('Hello Binary Files'));
	...
	test('encoding: decode', async function () {
		...
		// without setting
		assert.strictEqual(await vscode.workspace.decode(Uint8Array.from(Buffer.from('Hello World')), uri), 'Hello World');
		assert.strictEqual(await vscode.workspace.decode(Uint8Array.from(Buffer.from('Hellö Wörld')), uri), 'Hellö Wörld');
		...
		assert.strictEqual(await vscode.workspace.decode(Uint8Array.from(Buffer.from('Hello World')), uri, { encoding: 'foobar123' }), 'Hello World');
