...
suite('vscode API - workspace-fs', () => {
	...
	test('fs.write/stat/read/delete', async function () {
		...
		await vscode.workspace.fs.writeFile(uri, Uint8Array.from(Buffer.from('HELLO')));

		const stat = await vscode.workspace.fs.stat(uri);
		assert.strictEqual(stat.type, vscode.FileType.File);

		const contents = await vscode.workspace.fs.readFile(uri);
		assert.strictEqual(Uint8Array.from(Buffer.from(contents)).toString(), 'HELLO');
	...
	test('fs.delete folder', async function () {
		...
		await vscode.workspace.fs.writeFile(file, Uint8Array.from(Buffer.from('FOO')));
	...
	test('fs.createFolder creates recursively', async function () {
		...
		await vscode.workspace.fs.writeFile(file, Uint8Array.from(Buffer.from('Hello World')));
