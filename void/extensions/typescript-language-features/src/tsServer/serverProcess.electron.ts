...
class ProtocolBuffer {
	...
	public append(data: string | Buffer): void {
		...
		if (this.buffer.length - this.index >= toAppend.length) {
			this.buffer.set(toAppend.subarray(0, toAppend.length), this.index);
		} else {
			const newSize = (Math.ceil((this.index + toAppend.length) / defaultSize) + 1) * defaultSize;
			if (this.index === 0) {
				this.buffer = Buffer.allocUnsafe(newSize);
				this.buffer.set(toAppend.subarray(0, toAppend.length), 0);
			} else {
				this.buffer = Buffer.concat([Uint8Array.from(this.buffer.subarray(0, this.index)), Uint8Array.from(toAppend)], newSize);
			}
		}
	...
	public tryReadContent(length: number): string | null {
		...
		this.buffer.copyWithin(0, sourceStart);
