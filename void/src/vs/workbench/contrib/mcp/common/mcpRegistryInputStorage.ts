	...
	private async _sealSecrets() {
		...
		return this._secretsSealerSequencer.queue(async () => {
			...
			const encrypted = await crypto.subtle.encrypt(
				{ name: MCP_ENCRYPTION_KEY_ALGORITHM, iv },
				key,
	...
	private async _unsealSecrets(): Promise<Record<string, IResolvedValue>> {
		...
		try {
			...
			const decrypted = await crypto.subtle.decrypt(
				{ name: MCP_ENCRYPTION_KEY_ALGORITHM, iv: new Uint8Array(iv.buffer as unknown as ArrayBuffer) },
				key,
				new Uint8Array(encrypted.buffer as unknown as ArrayBuffer),
			);
